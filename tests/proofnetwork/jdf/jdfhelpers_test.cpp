...
TEST_F(JdfHelpersTest, findAllNodes)
{
    ...
    const QStringList ids = {"root",
                             "cutting_node3",
                             "cutting_node2",
                             "cutting_and_folding_node1",
                             "folding_node3",
                             "folding_node2",
                             "folding_and_boxpacking_node1",
                             "boxpacking_node3",
                             "boxpacking_node2"};
    ASSERT_EQ(ids.count(), nodes.count())
        << Proof::algorithms::map(nodes, [](const auto &n) { return n->id(); }, QStringList()).join(", ").toStdString();
...
TEST_F(JdfHelpersTest, probablyFoldingNodes)
{
    ...
    const QStringList ids = {"cutting_and_folding_node1", "folding_and_boxpacking_node1", "folding_node2",
                             "folding_node3"};
    ASSERT_EQ(ids.count(), nodes.count())
        << Proof::algorithms::map(nodes, [](const auto &n) { return n->id(); }, QStringList()).join(", ").toStdString();
    for (int i = 0; i < nodes.count(); ++i)
