    ...
    if (type.isEmpty()) {
        ...
        return job->findAllNodes([type = type.toLower()](const JdfNodeSP &node) -> bool {
            ...
            if (loweredType == QLatin1String("processgroup") || loweredType == QLatin1String("combined"))
                return algorithms::findIf(node->types(), [type](const auto &t){return !t.compare(type, Qt::CaseInsensitive);}).count();
            return false;
        });
    }
}
...
inline QList<JdfNodeSP> nodesWithLink<CuttingParams>(const QList<JdfNodeSP> &nodes, LinkUsage usage)
{
    return algorithms::filter(nodes, [usage](const JdfNodeSP &node)->bool {
        return node->resourceLinkPool()->cuttingParamsLink()->isDirty() && node->resourceLinkPool()->cuttingParamsLink()->usage() == usage;
    });
}
...
{
    return algorithms::filter(nodes, [usage](const JdfNodeSP &node)->bool {
        return node->resourceLinkPool()->foldingParamsLink()->isDirty() && node->resourceLinkPool()->foldingParamsLink()->usage() == usage;
    });
}
...
{
    return algorithms::filter(nodes, [usage](const JdfNodeSP &node)->bool {
        return node->resourceLinkPool()->boxPackingParamsLink()->isDirty() && node->resourceLinkPool()->boxPackingParamsLink()->usage() == usage;
    });
}
...
{
    return algorithms::filter(nodes, [usage](const JdfNodeSP &node)->bool {
        return algorithms::findIf(node->resourceLinkPool()->componentLinks(), [usage](const auto &link){return link->usage() == usage;});
    });
}
...
{
    return algorithms::filter(nodes, [usage](const JdfNodeSP &node)->bool {
        return algorithms::findIf(node->resourceLinkPool()->mediaLinks(), [usage](const auto &link){return link->usage() == usage;});
    });
}
